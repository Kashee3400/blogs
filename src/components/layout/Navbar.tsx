            ...
            <div className="flex-shrink-0">
              <Link to="/" className="flex items-center space-x-3 group">
                <div className="w-10 h-10 lg:w-12 lg:h-12 bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-105 transition-transform duration-300">
                  <span className="text-white font-bold text-lg lg:text-xl">LB</span>
                </div>
                <div className="hidden sm:block">
                  ...
                  <p className="text-xs text-gray-500 -mt-1">Make your Life beautiful. </p>
                </div>
              </Link>
            </div>
            ...
            <div className="hidden lg:flex items-center space-x-1">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  to={item.href}
                  className={`group px-4 py-2.5 rounded-xl font-medium transition-all duration-300 hover:bg-indigo-50 relative overflow-hidden ${isActiveRoute(item.href)
                ...
                >
                  ...
                  <div className={`absolute bottom-0 left-0 h-0.5 bg-gradient-to-r from-indigo-600 to-purple-600 transition-all duration-300 ${isActiveRoute(item.href) ? 'w-full' : 'w-0 group-hover:w-full'
                    }`}></div>
                </Link>
              ))}
                    ...
                    <div className="p-2">
                      {moreMenuItems.map((item, index) => (
                        <Link
                          key={item.name}
                          to={item.href}
                          className={`group flex items-center space-x-4 p-4 rounded-xl transition-all duration-300 hover:scale-[1.02] ${isActiveRoute(item.href)
                            ? 'bg-indigo-50 text-indigo-600'
                            : 'hover:bg-indigo-50'
                            }`}
                          style={{ animationDelay: `${index * 50}ms` }}
                        >
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 ${isActiveRoute(item.href)
                          ...
                          <div className="flex-1">
                            <div className={`font-semibold transition-colors duration-300 ${isActiveRoute(item.href)
                              ? 'text-indigo-600'
            ...
            <div className="px-4 py-6 space-y-2">
              {navItems.map((item, index) => (
                <Link
                  key={item.name}
                  to={item.href}
                  className={`flex items-center space-x-3 px-4 py-3 rounded-xl font-medium transition-all duration-300 ${isActiveRoute(item.href)
                    ? 'text-indigo-600 bg-indigo-50'
              ...
              <div className="pt-4 border-t border-gray-200/20">
                ...
                {moreMenuItems.map((item) => (
                  <Link
                    key={item.name}
                    to={item.href}
                    className={`flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-300 ${isActiveRoute(item.href)
                      ? 'text-indigo-600 bg-indigo-50'
                      : 'text-gray-700 hover:text-indigo-600 hover:bg-indigo-50'
                      }`}
                  >
